...
const AMMO_STATIC_COLLECT_RADIUS = 1.65;
const AMMO_STATIC_COLLECT_HEIGHT = 2.5;
...
export function spawnStaticAmmoCollectible(scene, x, y, z, floorY, value = AMMO_DROP_VALUE) {
  ...
  mesh.position.set(x, floorY + AMMO_DROP_SETTLE_Y, z);
  scene.add(mesh);
...
export function updateAmmoDrops(drops, dt, playerPos, onCollect, colliders, bounds, floorHoles = []) {
  for (let i = drops.length - 1; i >= 0; i--) {
    ...
    if (!d.collected) {
      ...
      if (dx * dx + dz * dz < collectRadius * collectRadius) {
        if (d.permanent) {
          const footY = playerPos.y - PLAYER_STAND_EYE;
          if (footY < d.floorY - 0.35 || footY > d.floorY + AMMO_STATIC_COLLECT_HEIGHT) {
            continue;
          }
        }
        d.collected = true;
