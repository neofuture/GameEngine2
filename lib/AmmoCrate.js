...
import {
  getShadowDepthMaterial,
  getShadowOccluderMaterial,
} from "./ShadowOccluders.js";
import {
...
/** Solid box in world space — sibling of the visible crate, not a scaled child. */
function createPickupShadowCaster(mesh) {
  ...
  caster.customDepthMaterial = getShadowDepthMaterial();
  ...
  caster.userData.pickupWorldScale = scale;
...
function syncPickupShadowCaster(mesh) {
  ...
  const base = caster.userData.pickupWorldScale ?? AMMO_DROP_SCALE;
  const s = mesh.scale.x / base;
  caster.scale.set(s, s, s);
}

function ensurePickupShadowCaster(mesh) {
  if (mesh.userData.pickupShadowCaster) {
    syncPickupShadowCaster(mesh);
    return;
  }
  const caster = createPickupShadowCaster(mesh);
  mesh.userData.pickupShadowCaster = caster;
  if (mesh.parent) mesh.parent.add(caster);
  syncPickupShadowCaster(mesh);
}
