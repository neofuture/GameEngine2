...
export function shouldSkipCollider(
  ...
  if (stairLocal) {
    ...
    /** Arena floor + moving up the flight (+localZ) — the only case under-slope blockers should be solid. */
    const onArenaFloor = footY <= floorY + 0.18;
    const cheatWalkUnder =
      onArenaFloor &&
      climbLocalMotion > 0.25 &&
      stairLocal.localZ > -1.85 &&
      stairLocal.localZ < 5.45;
    ...
    if (box.kind === "stairBack") {
      ...
      if (!cheatWalkUnder && stairLocal.localZ > -1.85) return true;
      if (climbLocalMotion <= 0.12 && stairLocal.localZ > -1.85) return true;
      if (approachingLip || leavingLip) return true;
      if (onRampSurface || withinStepUpOfRamp) return true;
    }
...
export function clampStairUnderForward(
  ...
  const onArenaFloor = footY <= floorY + 0.18;
  if (!onArenaFloor || climbLocalMotion <= 0.25 || footY + 0.15 >= (rampFootY ?? 0)) {
    return;
  }
