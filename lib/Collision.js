 ...
 * @param {number} [floorY=0] Arena floor walk height — used to detect floor-level walk-under cheats
 */
export function shouldSkipCollider(
  ...
  if (stairLocal) {
    const underStairKind =
      box.kind === "stairBack" ||
      box.kind === "stairRearCurtain" ||
      box.kind === "stairBackSlice";

    if (underStairKind) {
      /** Feet on the ramp slope — under-carriage colliders don't apply. */
      const onRampSurface =
        followingRamp ||
        (rampFootY != null && footY >= rampFootY - 0.15);

      if (onRampSurface) return true;

      /** Stepping onto the bottom tread from the arena floor (front approach only). */
      const frontLipApproach =
        footY <= floorY + 0.18 &&
        climbLocalMotion > 0.12 &&
        stairLocal.localZ >= -0.08 &&
        stairLocal.localZ <= 0.42;

      if (frontLipApproach && box.kind !== "stairBack") return true;

      const withinStepUpOfRamp =
        rampFootY != null &&
        footY >= rampFootY - 0.48 &&
        stairLocal.localZ >= -0.12;

      if (withinStepUpOfRamp) return true;

      /** Rear shell — solid behind the flight; skip once clearly in front of it. */
      if (box.kind === "stairBack" && stairLocal.localZ >= -0.05) return true;

      /** Descending off the bottom lip toward the arena floor. */
      if (
...
/**
 * Floor-level walk-under cheats slip through gap-to-gap when boxes use shortest-face
 * push-out. Clamp the player behind the foremost blocking under-slope slice instead.
 *
 * @param {{ x: number, z: number }} position
 * @param {number} radius
 * @param {ColliderBox[]} colliders
 ...
 * @param {number} bodyTop
 * @param {number} floorY
 * @param {number} climbLocalMotion
 * @param {number | null} rampFootY
 * @param {boolean} followingRamp
 * @param {number} supportY
 * @param {(flight: object, x: number, z: number) => { localX: number, localZ: number }} worldToStairLocal
 * @param {THREE.Vector3} scratch
 */
export function clampStairUnderForward(
  position,
  radius,
  colliders,
  footY,
  ...
  const onArenaFloor = footY <= floorY + 0.18;
  if (!onArenaFloor || climbLocalMotion <= 0.25 || followingRamp) {
    return;
  }
  if (rampFootY != null && footY + 0.15 >= rampFootY) {
    return;
  }
  ...
  for (const box of colliders) {
    ...
    if (box.sliceBackLocalZ == null || !box.stairFlight) continue;

    const stairLocal = worldToStairLocal(
      box.stairFlight,
      position.x,
      position.z
    );
    const halfW =
      box.stairFlight.walkHalfWidth ??
      box.stairFlight.ramp?.halfWidth ??
      1.75;
    if (Math.abs(stairLocal.localX) > halfW + radius) continue;
    ...
    const sliceForwardLocalZ =
      box.sliceForwardLocalZ ?? box.sliceBackLocalZ + 1;
    if (stairLocal.localZ <= box.sliceBackLocalZ - radius) continue;
    if (stairLocal.localZ > sliceForwardLocalZ + radius) continue;

    maxLocalZ = Math.min(maxLocalZ, box.sliceBackLocalZ - radius);
    flight = box.stairFlight;
  }

  if (maxLocalZ === Infinity || !flight?.inverseMatrix) return;

  const local = worldToStairLocal(flight, position.x, position.z);
  if (local.localZ <= maxLocalZ) return;

  scratch.set(local.localX, 0, maxLocalZ);
  scratch.applyMatrix4(flight.matrixWorld);
