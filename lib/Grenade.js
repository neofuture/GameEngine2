...
function cloneGrenadeModel(anisotropy = 8) {
  ...
  group.add(new THREE.Mesh(_sharedBodyGeo, _sharedBodyMat));

  const topCap = new THREE.Mesh(_sharedTopGeo, _sharedTopMat);
  topCap.rotation.x = -Math.PI / 2;
  topCap.position.y = _capTopY;
  group.add(topCap);

  const botCap = new THREE.Mesh(_sharedBotGeo, _sharedBotMat);
