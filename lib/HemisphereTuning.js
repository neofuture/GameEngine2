...
/** Cool overcast daylight bias + moderate intensity. */
export const DEFAULT_HEMI_DAY = Object.freeze({
  temperature: 11000,
  intensity: 0.32,
});

/** Cold blue cast at very low intensity so geometry never goes pitch black. */
 ...
 */
export function applyHemisphereSettings(hemi, settings) {
  if (!hemi?.isHemisphereLight) return;
  const rgb = kelvinToRgb(settings.temperature);
  hemi.color.setRGB(rgb.r, rgb.g, rgb.b);
  // Darken + desaturate the bounce: 42% luminance, blended 30% toward grey.
  const lum = 0.42;
  const grey = 0.3;
  const gr = rgb.r * lum;
  const gg = rgb.g * lum;
