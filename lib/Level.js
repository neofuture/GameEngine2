...
function addArenaCeilingDeckPiece(
  ...
  if (tileSize) {
    applyWorldSpaceDeckUVs(geometry, minX, maxX, minZ, maxZ, thickness, tileSize);
  }
...
/**
 * Lock deck UVs to world X/Z so moving the stair cutout never shifts the
 * ceiling texture — only the hole geometry changes.
 */
function applyWorldSpaceDeckUVs(
  geometry,
  minX,
  maxX,
  minZ,
  ...
  for (let i = 0; i < pos.count; i++) {
    // Underside (-Y) is what the player sees when looking up at the catwalk.
    if (pos.getY(i) > -halfH + eps) continue;

    const wx = cx + pos.getX(i);
    const wz = cz + pos.getZ(i);
    uv.setXY(i, wx / tileSize, -wz / tileSize);
  }
  uv.needsUpdate = true;
}
