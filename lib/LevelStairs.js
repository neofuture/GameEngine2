...
export function buildStairFlight(
  ...
  // Walk support matches the ceiling-deck cutout (same hole the mesh clears).
  const cutoutHalfW = STAIRS_WIDTH / 2 + STAIR_STRINGER_WIDTH;
  const cutoutRearZ = -STAIR_STRINGER_DEPTH_OVERHANG;
  const cutoutFrontZ = STAIRS_TOTAL_RUN + STAIR_STRINGER_DEPTH_OVERHANG;
  const cutoutFootprint = localFootprintWorldBounds(
    flight,
    0,
    (cutoutRearZ + cutoutFrontZ) / 2,
    cutoutHalfW,
    (cutoutFrontZ - cutoutRearZ) / 2
  );
  groundSurfaces.push({
    minX: cutoutFootprint.minX,
    maxX: cutoutFootprint.maxX,
    minZ: cutoutFootprint.minZ,
    maxZ: cutoutFootprint.maxZ,
    y: topWalkWorldY,
    edgeStandoff: catwalkEdgeStandoff ?? undefined,
    catwalkWalk: true,
  });
