...
export function buildStairFlight(
  ...
  const treadHalfDepth = (STAIRS_STEP_RUN + 0.06) / 2;

  // Far rear bulkhead — behind the spawn-side approach (localZ ≲ −1.05).
  const backHalfZ = 0.075;
  const backLocalZ = -1.38;
  const backCenter = localCenterToWorldXZ(flight, 0, backLocalZ);
  pushCollider(colliders, {
    x: backCenter.x,
    z: backCenter.z,
  ...
  // Curtain behind the bottom tread lip (blocks ground-level walk-through).
  ...
  // Rear-half tread slices (i ≥ 1) — block walking under the slope at floor height.
  for (let i = 1; i < STAIRS_STEP_COUNT; i++) {
    const treadCenterZ = STAIRS_STEP_RUN * (i + 0.5);
    const sliceBackLocalZ = treadCenterZ - treadHalfDepth;
    const sliceForwardLocalZ = treadCenterZ;
    const sliceCenterZ = treadCenterZ - treadHalfDepth / 2;
