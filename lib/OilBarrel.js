...
function ensureBodyMaterial() {
  if (!_bodyMat) {
    _bodyMat = new THREE.MeshStandardMaterial({ side: THREE.DoubleSide });
...
function makeBodyMaterial(tex, repeatU, repeatV) {
  ...
  mat.map = configureColorTex(tex.bodyAlbedo.clone(), repeatU, repeatV);
  mat.normalMap = configureDataTex(tex.bodyNormal.clone(), repeatU, repeatV);
  mat.emissiveMap = configureColorTex(tex.bodyEmissive.clone(), repeatU, repeatV);
...
function applyRimWallAlignedUVs(
  ...
  for (let i = 0; i < pos.count; i++) {
    ...
    const u = phiIndex / radialSegments;
    const y = pos.getY(i);
    const v = wallEdgeV + y / wallHeight;
    uv.setXY(i, u, v);
  }
  uv.needsUpdate = true;
}

/** @param {number} radius @param {number} bevel @param {1 | -1} towardCap @param {number} segments @param {number} wallHeight */
function getRimFilletGeo(radius, bevel, towardCap, segments, wallHeight) {
  const profile = buildRimFilletProfile(radius, bevel, towardCap);
  const key = `${radius}|${bevel}|${towardCap}|${segments}|${wallHeight}`;
  let geo = _rimGeoCache.get(key);
  if (!geo) {
    geo = new THREE.LatheGeometry(profile, segments);
    applyRimWallAlignedUVs(geo, wallHeight, towardCap, segments, profile.length);
...
function buildBarrelMesh(options = {}) {
  ...
  const bodyMat = ensureBodyMaterial();

  const wall = new THREE.Mesh(
    getWallCylinderGeo(r, wallHeight, radialSegments),
    bodyMat
  );
  wall.name = "oil_barrel_wall";

  const bottomRim = new THREE.Mesh(
    getRimFilletGeo(r, bevel, -1, radialSegments, wallHeight),
    bodyMat
  );
  bottomRim.name = "oil_barrel_rim_bottom";
  ...
  topRim.name = "oil_barrel_rim_top";
