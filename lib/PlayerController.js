...
export function createPlayerController(camera, bounds, floorY, options = {}) {
  ...
  let footstepPhase = 0;
  let footstepStrideActive = false;
  ...
  function updateWalkBob(horizontalSpeed, crouching, aiming, dt) {
    ...
    if (onFootstep && canStep) {
      if (!footstepStrideActive) {
        footstepStrideActive = true;
        footstepPhase = 0;
        onFootstep({
      ...
      const prevHalf = Math.floor(footstepPhase / Math.PI);
      footstepPhase += dt * bobFreq * Math.PI * 2;
      const curHalf = Math.floor(footstepPhase / Math.PI);
      for (let half = prevHalf + 1; half <= curHalf; half += 1) {
        onFootstep({
          speed: horizontalSpeed,
          crouching,
          sprinting: isSprinting,
  ...
  function bestFlatSupportAt(x, z, footY) {
    ...
    for (const [dx, dz] of samples) {
      ...
      for (const surf of surfaces) {
        if (surf.stairFlight) continue;
        if (pointOnFlatSurface(sx, sz, footY, surf)) {
