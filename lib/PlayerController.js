    ...
    for (const surf of getGroundSurfaces()) {
      if (surf.stairFlight) {
        ...
        if (hit === "standing") {
          ...
          // Don't promote a tread we can't actually fit on top of with our
          // current body height — otherwise the step's wall collider would
          // get skipped (step-up exception in shouldSkipCollider) and we'd
          // ramp / snap straight into the ceiling above the tread.
          if (!hasHeadroom(x, z, surf.y, eyeHeight + 0.02)) continue;
  ...
  return {
    update(input, dt) {
      ...
      const canStand = hasHeadroom(
        ...
        position.z,
        headroomFootY,
        STAND_EYE + STAND_CLEARANCE_MARGIN
      );
      ...
      if (position.y <= groundLevel) {
        ...
        if (isStepUp) {
          // Soften the vertical interpolation on stairs so each tread
          // doesn't snap to its top — feels more like riding a ramp than
          // popping up to each step. The base stepUpSmooth still applies
          // when stepping over a one-off curb.
          const stepUpSmooth = onStairs
            ? bobTuning.stepUpSmooth * 0.45
            : bobTuning.stepUpSmooth;
          position.y +=
            riseNeeded * (1 - Math.exp(-stepUpSmooth * dt));
      ...
      if (
        ...
        // Stairs trigger this spring on every tread. At full strength that
        // stacks into a noticeable bounce per step — pull the impulse way
        // down on stairs so the climb stays gentle while a one-off curb
        // step still feels weighty.
        const stairAtten = onStairs ? 0.22 : 1;
        const stepHeight = supportY - prevSupportY;
        stepBobY = -Math.min(
          bobTuning.stepDip * stairAtten,
          stepHeight * 0.22 * stairAtten
        );
        stepBobVel = stepHeight * bobTuning.stepKick * stairAtten;
      }
