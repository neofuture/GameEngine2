...
export function createPlayerController(camera, bounds, floorY, options = {}) {
  ...
  function findCatwalkWalkBounds(footY, x, z) {
    ...
    let onCatwalk = onDeck || onStairAtCatwalk;
    if (!onCatwalk) {
    ...
    for (const surf of surfaces) {
      if (!isCatwalkSurface(surf, footY, ySlack)) continue;
    ...
    if (rampY != null) {
      ...
      if (deckY != null) {
        ...
        if (
          ...
          return { supportY: stepUpFlat, onStairs: false, stairRamp: false };
        }
