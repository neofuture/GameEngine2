...
export function createPlayerController(camera, bounds, floorY, options = {}) {
  ...
  function findCatwalkWalkBounds(footY, x, z) {
    ...
    for (const surf of getGroundSurfaces()) {
      ...
      if (surf.y == null || Math.abs(surf.y - footY) > ySlack) continue;
      if (surf.minX == null) continue;
      if (!surf.edgeStandoff && !surf.catwalkWalk) continue;

      minX = Math.min(minX, surf.minX);
      maxX = Math.max(maxX, surf.maxX);
      minZ = Math.min(minZ, surf.minZ);
      maxZ = Math.max(maxZ, surf.maxZ);
      if (pointInSurfaceBounds(x, z, surf)) onCatwalk = true;
      if (surf.edgeStandoff) edgeStandoff = surf.edgeStandoff;
    }

    if (!onCatwalk && Number.isFinite(minX)) {
    ...
    if (!onCatwalk || !Number.isFinite(minX)) return null;
  ...
  function resolveBounds() {
    ...
    const inRoom = arenaBounds ? getIsInRoom(position.x, position.z) : false;
    const footY = position.y - eyeHeight;
    const catwalkBounds =
      !inRoom ? findCatwalkWalkBounds(footY, position.x, position.z) : null;
