...
export function createPlayerController(camera, bounds, floorY, options = {}) {
  ...
  const initialPosition = options.initialPosition
    ? new THREE.Vector3(
        ...
        options.initialPosition.y ?? STAND_EYE,
        options.initialPosition.z ?? 6
      )
    : new THREE.Vector3(0, STAND_EYE, 6);
  ...
  function getSupportInfo(x, z, footY, horizontalSpeed = 0) {
    ...
    // When the (x, z) sample is inside a circular floor cutout there's no
    // implicit floor support — use -Infinity so any actual ground surface
    // (e.g. a catwalk above) still wins, but if nothing supports the
    // player they keep falling instead of being clamped to floorY.
    let bestFlat = pointInFloorHole(x, z)
