...
const MOON_DISC_SIZE = CELESTIAL_RADIUS * 0.22;
...
export async function createSkyDome(scene, options = {}) {
  ...
  // Real LRO color mosaic on a small sphere. MeshBasicMaterial keeps the
  // moon fully lit regardless of scene lighting (no shadow-casting from the
  // directional lights — its directional light models *moonlight reaching
  // the world*, not the moon's own appearance). The sphere is oriented per
  // frame so its prime meridian (the recognisable near side with Mare
  // Imbrium / Tranquillitatis) faces the camera.
  ...
  const moonGeometry = new THREE.SphereGeometry(moonRadius, 48, 32);
  ...
  const moonHaloSprite = new THREE.Sprite(moonHaloMaterial);
  moonHaloSprite.name = "sky_moon_halo";
  // Halo extends only ~20% past the disc on each side — a tight rim glow,
  // not a sky-wide bloom. (At sky distance 180 with the current moon disc
  // size, that's ~20px of visible ring around the moon.)
  const haloSize = MOON_DISC_SIZE * 1.4;
  moonHaloSprite.scale.set(haloSize, haloSize, 1);
  moonHaloSprite.userData.isSkyDome = true;
  moonHaloSprite.frustumCulled = false;
  moonHaloSprite.renderOrder = -999;
  setWorldLayer(moonHaloSprite);
  root.add(moonHaloSprite);

  // Aim the moon's near side (prime meridian, longitude 0°) at the camera.
  // The sky root sits at the camera position with no rotation, so the
  // camera in sky-local space is the origin and the desired "forward" for
  // the moon — its local +X axis (where SphereGeometry's default UV places
  // u=0.5) — should point from the moon back to the origin.
  ...
  function orientMoonToCamera() {
    ...
    if (_moonRight.lengthSq() < 1e-6) {
      _moonRight.set(0, 0, 1);
    } else {
      _moonRight.normalize();
    }
    _moonAdjUp.crossVectors(_moonForward, _moonRight).normalize();
    // Columns of the basis are (local +X, +Y, +Z) in parent (sky-root) space.
    _moonBasis.makeBasis(_moonForward, _moonAdjUp, _moonRight);
    moonMesh.quaternion.setFromRotationMatrix(_moonBasis);
  }
  ...
  return {
    ...
    setMoonPosition(pos) {
      ...
      placeCelestialSprite(moonHaloSprite, pos);
      orientMoonToCamera();
    },
    ...
    setMoonOpacity(opacity) {
      ...
      // Halo at 40% of the disc opacity — a subtle rim glow that frames
      // the moon without competing with the photo.
      moonHaloMaterial.opacity = o * 0.4;
    },
    update(camera) {
