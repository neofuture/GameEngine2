...
export function setTargetHealthVisual(mesh, healthRatio) {
  ...
  if (alive) {
    mat.emissive.setHex(ragEmissiveHex(ratio));
    mat.emissiveIntensity = TARGET_EMISSIVE_INTENSITY;
  } else {
...
export function applyTargetHit(mesh) {
  ...
  // Restart the "show on hit" window — hold then fade handled per frame.
